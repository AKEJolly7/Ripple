...
- 技术栈：Java 17 · langchain4j（OpenAI 兼容 → DeepSeek）· Apache POI · ECharts 本地打包。

## 本仓约定（仅此 5 条）

1. **构建/测试**：本机默认 java/mvn 都指向 JDK 8（`~/.mavenrc` 强制 zulu-8，勿改，其他项目依赖它）。
   构建前必须：`export JAVA_HOME=~/tools/jdk-17/Contents/Home MAVEN_SKIP_RC=1`
   （JDK 17 已装在 ~/tools，MAVEN_SKIP_RC=1 让 mvn 跳过 ~/.mavenrc 的 JAVA_HOME 覆盖）；
...
4. **产物目录**：HTML 报告输出到 `output/`，任务 B 三件套（Excel/PPT/Word）输出到 `artifacts/`，
   数据抓取中间产物输出到 `work/`，三者随仓库提交作为溯源与验收凭据；提交前密钥扫描须覆盖已入库产物；
   `echarts.min.js`（SHA-256: b66b25ae…0fd0）之外禁止引入任何前端外部资源。
5. **开发日志**：每个里程碑（R0-R10）完成或中止时更新 `DEVLOG.md`（时间、内容、AI 参与部分、
